...
let currentMoveIndex = 1; // Index for the moveHistory array
let timeStamps = [];
...
function initWebSocket() {
    socket = new WebSocket(`ws://192.168.155.94:8080/watch-game/${gameID}`);
...
function handleServerMessage(message) {
    ...
    switch (type) {
        case 'info':
            ...
            currentTurn = tinkerTurn = currentMoveIndex % 2 ? 'w' : 'b';

            if(msgJson.timeStamps.length) {
                const stamps = msgJson.timeStamps.split(' ');

                stamps.forEach((stamp) => {
                    timeStamps.push([parseInt(stamp.substring(0, stamp.find('-') + 1)), parseInt(stamp.substring(stamp.find('-') + 1, stamp.length))]);
                });
            }
            ...
            addChatMessage("System", `${playerName} vs ${opponentName}`);
            break;
...
function onDragStart(source, piece) {
    tinkerTurn = currentMoveIndex % 2 ? 'w' : 'b';
